...
def format_spent(spent: str) -> list[str] | int:
    '''returns [product, amount, category]'''
    if ("," in spent):
        spent = spent.split(",")
        spent[1] = int(spent[1].replace(" ",""))
        spent[2] = spent[2].replace(" ","")
        
    else:
        spent = spent.split(" ")
        
    # check if spent = [product, amount, category]
    if len(spent) != 3:
        return 0
...
def get_spent(spent: list) -> Expense:
    '''Creates an expense object'''
    return Expense(item=spent[0], amount=spent[1], category=spent[2])
