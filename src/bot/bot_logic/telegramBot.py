...
class MoneyMate():
    ...
    async def clear(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        self.dataBase.clear_all_expenses()
        self.worksheet.sheet_clear()
    ...
    async def add_spending(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # format the message recieved
        formatted_spent = aux.format_spent(update.message.text)

        if formatted_spent == 0:
            await update.message.reply_text(f"🚫 Invalid format 🚫\nTry this format:\nproduct spent category\nyour product, spent, category")
            return None
        elif formatted_spent == 1:
            await update.message.reply_text(f"🚫 Invalid format 🚫\nAmount must be a number")
            return None
        
        # get an expense object
        spent = aux.get_spent(formatted_spent)
    ...
    async def spent(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            spents = aux.format_df_itemized_to_monospaced_table(spent, title)
    ...
    async def set_category_budget(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
